 ...
 public function verify_student()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($student) {
                ...
                $is_worker_child = ($student->PAYE == 5); // تحديث: PAYE=5
                $is_reeducation = ($student->FRAISINS == 1500);

                if (!$is_paid && !$is_worker_child && !$is_reeducation) {
                    $this->session->set_flashdata('upload_error', 'لا يمكنك إرفاق الوثائق. يجب عليك دفع حقوق التسجيل أولاً      .');
                    $this->_show_auth_page_with_error($code_annexe);
                    return;
                }
                ...
                // --- المنطق الجديد لتاريخ الميلاد ---
                $show_extra_fields = FALSE; // هل نظهر الحقول 5 و 6؟
                if (!$is_returning_student && !$is_reeducation) {
                    $birth_date_obj = new DateTime($student->DNS);
                    $cutoff_date = new DateTime('1997-01-01'); // التاريخ الجديد
                    if ($birth_date_obj < $cutoff_date) {
                        $show_extra_fields = true; // مولود قبل 1997: أظهر 6 حقول
                    }
                }
                ...
                $session_data = [
                    ...
                    'show_extra_fields' => $show_extra_fields // متغير جديد للجلسة
                ];
    ...
    {
        // التأكد أولاً أن المتعلم قام بتسجيل الدخول عبر الواجهة الأولى
        if (!$this->session->userdata('upload_student_enpr')) {
            redirect('upload_documents');
        }

        // جلب جميع المتغيرات من الجلسة
        ...
        // تجهيز البيانات لإرسالها للواجهة
        $data = [
            'title' => 'إرفاق الوثائق - الخطوة 3',
            'fich_js' => '', // لا حاجة لملف جافاسكريبت خاص هنا
            'is_returning' => $is_returning,
            'is_reeducation' => $is_reeducation,
            'show_extra_fields' => $show_extra_fields // <-- هذا هو المتغير الأهم
        ];
    ...
    private function _show_success_page($uploaded_urls)
    {
        ...
        // --- التعديل: إضافة بيانات المتعلم للوصل ---
        $data['student_info'] = [
            'enpr' => $this->session->userdata('upload_student_enpr'),
